...
export function AdminRow({ row, selfAdminId }: { row: StaffListRow; selfAdminId: string }) {
  const [name, setName] = useState(row.fullName);
  const [editingName, setEditingName] = useState(false);
  const [message, setMessage] = useState<{ kind: "ok" | "err"; text: string } | null>(null);
  const [isPending, startTransition] = useTransition();
  const isSelf = row.id === selfAdminId;

  function saveName() {
    setMessage(null);
    startTransition(async () => {
      const res = await renameAdminAction({ roleId: row.id, fullName: name });
      if (res.ok) {
  ...
  function reset() {
    setMessage(null);
    startTransition(async () => {
      const res = await resetAdminPasswordAction({ roleId: row.id });
      if (res.ok) {
        setMessage({
          kind: "ok",
  ...
  function toggleActive() {
    ...
    startTransition(async () => {
      const res = row.active
        ? await disableAdminAction({ roleId: row.id })
        : await enableAdminAction({ roleId: row.id });
    ...
    <tr className="border-b border-deh-border">
      ...
      <td className="px-3 py-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isSelf ? (
            <span className="text-deh-xs italic text-deh-muted">(you)</span>
          ) : (
            <Button
              size="sm"
